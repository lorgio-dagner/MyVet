...
namespace MyVet.Prism.ViewModels
{
    public class LoginPageViewModel : ViewModelBase
    {
        ...
        private async void Login()
        {
            ...
            // Aqui los pasos para consumir el owner
            var token = (TokenResponse)response.Result;
            // necesito el token por vamos a consumirnos la otra respuesta.
            var response2 = await _apiService.GetOwnerByEmailAsync(url, "api", "/Owners/GetOwnerByEmail", "bearer", token.Token, Email);
            if (!response2.IsSuccess)
            {
                IsRunning = false;
                IsEnabled = true;
                await App.Current.MainPage.DisplayAlert("Error", "This user have a big problem, call suport", "Accept");
                return;
            }
